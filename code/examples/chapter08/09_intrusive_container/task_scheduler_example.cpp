...
// 任务状态枚举
enum class TaskState { Ready, Running, Waiting, Done };
...
struct Task : IntrusiveListNode<Task> {
    ...
    Task(int i, const char* n, int p = 0) : id(i), name(n), state(TaskState::Ready), priority(p) {}
...
class TaskScheduler {
  public:
    void add_task(Task* task) { ready_list.push_back(task); }

    Task* get_next_task() { return ready_list.pop_front(); }

...
在 `main.cpp` 旁边，用步骤 2 那个「新建文件」图标，再建一个文件，名字叫 `CMakeLists.txt`（注意大小写，`C` `M` 大写，剩下小写,`Lists` 的 `L` 大写，剩下小写，后缀 `.txt`）。这个名字是 CMake 规定死的，差一个字母它都不认。
...
> 注意：
> cmake的项目不要放到中文路径下，不然会出现奇奇怪怪的问题(因为有人开发时使用只支持ASCII的API)，同时注意检查是否安装`mingw-make`，如果未选择编辑时的编译器可以`Ctrl+Shift+P` 打开命令面板，输入 `CMake: Select a Kit` 回车后进行选择

...
`NaN == NaN` 是 `false`，`NaN != NaN` 反而是 `true`。这是 IEEE 754 在标准层面定死的语义：`NaN` 代表「没有意义的运算结果」（$\frac{0}{0}$、$\sqrt{-1}$、$inf - inf$），「相等」对它没有定义，所以一律判不等。这就直接带来了一个写法陷阱——**想判 `NaN` 永远只能用 `std::isnan`，绝不能用 `==`**：
...
实际上 $\frac{1.0}{0.0}$ 得 $+inf$，$\frac{1.0}{-0.0}$ 得 $-inf$——零的符号会在除法里显灵，这也是为什么标准库要专门给一个 `signbit`。
...
### `hypot`：朴素 $\sqrt{x\times x+y\times y}$ 会溢出

求直角三角形斜边、或者向量的模长，直觉写法是 `std::sqrt(x*x + y*y)`。能用，但藏着一个数值溢出陷阱——当 `x` 和 `y` 本身没溢出，可 $x\times x$ 已经溢出成 `inf` 的时候，结果就废了。`std::hypot` 内部用了一套避免中间溢出的等价算法，专门救这个：
...
`1e200` 没溢出，但 $1e200 \times 1e200 = 1e400$ 远超 `double` 上限（约 $1.8e308$），平方这一步直接变成 `inf`，开方回来还是 `inf`，完全丢失了正确结果 $1.41421e+200$。`hypot` 算对了。C++17 起 `hypot` 还多了个三参数重载 `std::hypot(x, y, z)`，求三维向量模长同理。

教训：凡是要算模长、欧氏距离、$\sqrt{\sum x_i^2}$ 这种，别图省事写朴素平方和开方，直接上 `hypot`（高维用 `hypot` 链式或者更稳的算法）。除了防溢出，它对下溢（极小数平方变 0）也有保护，精度更稳。

### `fmod`：浮点取余，结果符号跟被除数

`std::fmod(x, y)` 是浮点版的取余，结果是 $x - n\times y$，其中 `n` 是「向零截断」的商。它和 `std::remainder`（C++11，向最近整数取余）符号规则不同，是个常见的混淆点：
...
`fmod(-5.3, 2.0)` 给 $-1.3$——结果的符号跟**被除数** `x` 走（因为商是向零截断的 $-2$，$-5.3 - (-2)\times2 = -1.3$）。而 `remainder(-5.3, 2.0)` 给 $0.7$——它取**最近的**整数商 $-3$，$-5.3 - (-3)\times2 = 0.7$。两者都对，但语义不同：要做周期映射（比如把角度归到 $[-pi, pi]$）通常想要 `remainder` 或 `fmod` 后再手动平移，搞混了会得到符号相反的结果。
...
$a \times b + c$ 这个再普通不过的式子，朴素写法要做两次运算、两次舍入（先算 $a\times b$ 舍入到 `double`，再加 `c` 再舍入一次）。`std::fma(a, b, c)`（C++99 从 C 借来，C++11 起标配）把这个表达式做成**单次**融合乘加（fused multiply-add）——中间的无限精度乘积直接加上 `c`，最后**只舍入一次**。
...
两者结果**不一样**：朴素写法给 `0`，`fma` 给 $-5.55e-17$。哪个「对」？取决于你怎么定义「对」——$\frac{1.0}{3.0}$ 在 `double` 里被截断成 `0.333...`（一个比真实 $\frac{1}{3}$ 略小的近似），朴素写法里 $\times 3$ 又把它舍入回了恰好 $1.0$（误差恰好被第二步乘法「修正」了），$-1$ 得 `0`；而 `fma` 不做中间舍入，老老实实反映了「`0.333... * 3` 略小于 1」这个事实，减 1 得到一个极小的负数。从「忠实反映中间近似值的真实数学结果」角度看，`fma` 更诚实；从「我希望 $\frac{x}{x}\times x$ 算出 `x`」的角度看，朴素反而更顺手。这正是浮点精度的微妙之处——**没有绝对正确的答案，只有「我清楚自己在要什么」**。
...
`std::fma` 的核心价值是**精度**（单次舍入，避免中间结果丢位），不是速度。在你的朴素 $a\times b+c$ 已经被编译器收缩成硬件 FMA 的常见场景下，显式 `std::fma` 反而可能更慢。需要 `fma` 的情况是：朴素计算出现了可见的精度损失（灾难性抵消、中间值溢出或丢位），并且你不能依赖编译器的自动收缩（开了 `-ffp-contract=off`，或者要跨平台严格可复现）。否则朴素写法既够准又够快。
:::
...
`beta(1,1)` 是 `1`（贝塔函数 $B(1,1) = \frac{\Gamma(1)\Gamma(1)}{\Gamma(2)} = 1$），`riemann_zeta(2)` 是 $1.6449...$（即著名的巴塞尔问题 $\frac{\pi^2}{6}$），`comp_ellint_2(0)` 是 $\frac{\pi}{2} \approx 1.5708$——都对得上数学预期。GCC 16.1.1 全部支持。
...
- **`hypot` 救溢出**：$\sqrt{x\times x+y\times y}$ 会在中间平方时溢出成 `inf`，`std::hypot` 用防溢出算法给出正确结果。
- **别用 `==` 比浮点**：累加误差让「理应相等」的数判不等。用绝对 + 相对 epsilon 的 `nearly_equal` 比较，阈值按场景定。
- **`fma` 保精度不一定提速**：`std::fma(a,b,c)` 单次舍入保精度，但 `-O2` 下朴素 $a\times b+c$ 常被自动收缩成硬件 FMA 还能向量化，反而更快。需要 `fma` 的是精度敏感、不能依赖编译器收缩的场景。

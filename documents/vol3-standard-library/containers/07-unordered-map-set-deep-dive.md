...
这里有个铁律：**hash 和 `==` 必须一致**。也就是说，如果 `a == b` 为真，那 `hash(a)` 必须等于 `hash(b)`——否则相等的元素会落到不同的桶，查找就找不到了。反过来不要求（`hash(a) == hash(b)` 时 `a` 不必等于 `b`，那只是冲突，正常现象）。上面这个 $x\times31 + y$ 是演示用的简单混合，生产环境可以用 `boost::hash_combine` 或更讲究的混合函数，进一步降低冲突概率。

...
好处:`sizeof` 小(紧凑),cache 友好。代价:**位运算**——读写位域成员是「读整字节 + 位掩码 + 位操作」,比读写普通 `int` 多几条指令。所以位域**省内存、费指令**。适合「有大量标志位、内存是瓶颈」(协议头、标志集合);不适合「单个字段被高频读写、算力是瓶颈」。Agner 卷1 §7.27 有详细权衡。
...
- **写代码时**:优先选零开销或近零开销的抽象(`enum class`、`span`/`string_view`、`optional`/`variant` 正常路径),它们让代码更安全且几乎不费性能。
- **关注时**:虚函数调用(via 指针,未被去虚化)、`std::function` 反复构造 + 大捕获、异常进热循环——这几个是真有成本、且常需要你手动优化的。
- **永远**:测了再优化。「听起来贵」的抽象可能编译器早消除了;「听起来免费」的(`std::function` 构造)可能正埋着堆分配。

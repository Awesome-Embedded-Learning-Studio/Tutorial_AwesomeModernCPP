...
- **两条屋顶线**:水平线是 CPU 峰值算力;斜线是峰值内存带宽($\frac{ops}{s} = \frac{bytes}{s} \times AI$,所以是一条过原点的斜线)。
...
- 算术强度 = $\frac{2 FLOP}{8 B} = 0.25 \frac{FLOP}{byte}$。

**AXPY `y[i] = α*x[i] + y[i]`**:

- 每次迭代:2 次浮点运算(乘、加),读 2 个 float + 写 1 个 float(12 字节)。
- 算术强度 = $\frac{2 FLOP}{12 B} ≈ 0.17 \frac{FLOP}{byte}$。

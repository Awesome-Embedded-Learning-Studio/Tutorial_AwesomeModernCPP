...
扩展性曲线是诊断多核程序的金标准:**跑一遍 1/2/4/8 核,画出来**。理想是一条直线(45° 斜率);弯下去拐平,就说明遇到了上面三个瓶颈之一。拐点在哪、拐得多狠,告诉你还有多少「加核能买到的性能」。
...
> **本机局限**:WSL2 在一台单 socket 笔记本上(5800H 单 NUMA 节点,`numactl --hardware` 只列出 node0),**测不了 NUMA 跨节点惩罚**。NUMA 的命令(`numactl`)和数据引自 Bakhvalov §11 + 多 socket 服务器实践。你要测 NUMA,得找一台双路服务器。本节诚实标注「本机测不了」。
...
绑核对**长运行、cache 敏感**的负载(数据库、流处理)很重要;对短任务意义不大(本来就来不及暖 cache)。绑核还能配合 NUMA(线程只在本 socket 的核上跑)。

> ch01-03「测量陷阱」里第 5/8 条(绑核、NUMA)就是这一节的内容,这里展开讲它们的原理。绑核是**性能测量**的标准动作(避免迁移噪声),也是**生产部署**的标准动作(稳定 cache 行为)。
...
- **扩展性曲线**:1/2/4/8 核跑同一任务,看加速比拐不拐平。理想线性,弯了说明撞了 Amdahl / 共享资源 / NUMA。
- **memory-bound 任务扩展性天然差**(共享内存带宽先打满),compute-bound 才好扩展。
- **Amdahl(固定规模)vs Gustafson(规模随核扩大)**:先想清你关心哪个。
- **NUMA**:多 socket 服务器,线程和数据要绑同 socket(`numactl`)。WSL2 单节点测不了。
- **affinity**:绑核减少迁移,测量和生产都该用。
- **线程不免费**:热路径用线程池,栈大小可调。

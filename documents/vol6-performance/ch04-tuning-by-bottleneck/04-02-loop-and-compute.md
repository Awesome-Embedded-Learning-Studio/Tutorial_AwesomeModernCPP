...
现代编译器对纯函数(`length()` 这种)const-fold + LICM(循环不变量外提)做得很好,大多数情况**自动外提**。但**编译器外提的前提是它能证明「这玩意不变」**——`volatile`、有副作用、可能修改全局状态的函数调用,它都不敢动。我们实测(`volatile float scale`,迫使编译器不能外提 vs 普通变量能外提):
...
把 `for (i) a[i]` 改成 `for (i+=4) { a[i]; a[i+1]; a[i+2]; a[i+3]; }`。两方面的好处:减少循环控制开销(分支、计数器自增)、给寄存器分配和 ILP 更多余地。现代编译器 `-O3 -funroll-loops` 会做,而且编译器选展开因子比手写聪明(能权衡寄存器压力),**默认情况别手写展开**。手写展开只在两种情况值得:(a) 你同时要做「多累加器」(展开+多累加器常一起写);(b) 编译器没展开但你 benchmark 证明展开有用。盲手展开经常**因为增加代码体积、icache miss 反而变慢**。

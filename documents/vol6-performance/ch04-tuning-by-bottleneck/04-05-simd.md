...
- **SIMD 理论 ×4/×8/×16**,实测手写 AVX2 点积拿 **~20×**(SIMD 宽度 + FMA + 多累加器叠加)。
- **自动向量化条件**:循环次数可估、无别名、无复杂分支。**现代 GCC(GCC 10+)默认会用 ordered 模式向量化 FP 归约(保持顺序、结果位精确),但单累加器限死依赖链**——拿满 SIMD 要靠**多累加器**(手写,或 `-ffast-math` 让编译器敢拆通道),不是「开 -O3/-ffast-math」就行。用 `-fopt-info-vec` 读诊断。
- **手写 intrinsics** 榨干 SIMD,但可移植性差、只在热点 + 自动做不到时上。
- **AVX-512 有降频坑**,SSE/AVX2+FMA 是安全甜点;必须 benchmark。
- **CPU 分发**(`target_clones` / `__builtin_cpu_supports`)解决可移植性;`std::simd`(C++26)是未来。

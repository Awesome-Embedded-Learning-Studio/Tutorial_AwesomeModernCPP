...
**重要变式**：教材示例里有一句「乱序也没问题」——那是 **C99** 的语义。**C++20 规定指定初始化器必须按声明顺序写**。写一个乱序的初始化（比如 `UART c{.parity = 0, .baudrate = 115200, ...}`），编译贴出真实报错。这一处教材表述与 C++20 标准不符，你的实测结果才是标准答案——本题就是要你把这条纠正记牢。

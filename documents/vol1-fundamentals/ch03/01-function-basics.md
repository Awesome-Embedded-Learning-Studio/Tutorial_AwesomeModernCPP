...
> 欸！那问题来了，一串函数咋办呢？全部写在开头吗？答案是不用，我们有 `#include` 功能。等下，我们把所有的声明放在一起，丢到一个文件。之后就不用到处手动的拷贝来拷贝去函数的声明了！多好。
>
> 恭喜你，你发明了头文件哈哈哈哈哈哈哈！
>
> 调侃之余，还是要说：这种"先声明后定义"的模式在实际项目中非常重要——我们后续学到头文件的时候就会看到，声明通常放在 `.h` 文件里供多个源文件共享，而定义放在 `.cpp` 文件中。现在只要记住一个原则就行：**编译器在使用一个函数之前，必须看到它的声明（或者定义）**。

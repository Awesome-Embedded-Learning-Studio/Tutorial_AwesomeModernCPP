...
**规则一：带默认值的参数只能连续出现在参数列表的末尾。** 从左往右读参数列表，一旦某个参数带上了默认值，它右边的所有参数也必须带默认值，中途不能断开。原因在调用语法：实参从左往右逐个填进形参，编译器只能靠"省略尾部参数"来决定哪些值用默认值。所以你不能跳过中间的参数——要给第三个参数传值，前面的所有参数都必须显式给出。落到接口设计上，参数的排列顺序就很重要：**把最常需要自定义的参数放在最左边，把几乎不会变的参数放在最右边**。
...
这一章我们学习了 C++ 在函数接口设计上的两个重要工具。函数重载允许同名函数根据参数类型和数量的不同表现出不同的行为，编译器通过一套严格的重载决议规则来决定最终调用哪个版本——精确匹配优先于类型提升，类型提升优先于标准转换，当两个候选函数无法分出高下时编译器直接报歧义错误。默认参数让调用者可以省略那些"几乎总是同一个值"的尾部参数，规则是带默认值的参数只能连续出现在参数列表末尾，且默认值只在声明处指定一次。两者各有擅长的领域——重载处理"类型不同"，默认参数处理"参数可选"——但混用时极易产生歧义，需要格外谨慎。

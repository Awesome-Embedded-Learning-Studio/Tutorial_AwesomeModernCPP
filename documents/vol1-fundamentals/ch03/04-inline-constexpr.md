...
`inline` 函数的定义必须出现在头文件中。如果你在头文件里只写了 `inline` 声明，而把定义放在 `.cpp` 文件里，其他翻译单元调用时找不到函数体，链接器要么报错，要么只能按普通函数调用。**定义必须和声明一起出现在头文件中**。
...
`constexpr` 不等于 `const`。`constexpr` 表示"编译期常量"（值在编译时确定），`const` 表示"运行时不可修改"（值在运行时确定但不能改变）。如果你需要编译期确定的值，用 `constexpr` 而不是 `const`。
...
`constexpr` 函数中如果包含浮点运算，编译期求值和运行时求值的结果可能存在微小差异——不同编译器、不同平台的浮点精度不完全一致。对于整数运算这不是问题，但如果你在 `constexpr` 函数中使用了浮点算法，最好用 `static_assert` 锁定期望结果。

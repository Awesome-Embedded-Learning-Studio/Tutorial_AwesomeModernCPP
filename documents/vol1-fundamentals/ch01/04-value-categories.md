...
值类别的区分我认为是一座小山峰，各位，准备好攀登咯！

我们已经跟变量、类型、const 打过不少交道了。但你有没有想过一个问题：为什么有些表达式能放在赋值号左边，有些就只能放在右边？为什么 `int& ref = x;` 编译得过，`int& ref = 42;` 就编译不过？不思考这些问题，咱们是很容易在后面的旅途中掉坑的！

值类别直接决定了编译器**怎么处理你写的每一个表达式**，哪些操作合法、哪些不合法，引用能绑定到什么上面，函数返回值怎么传递……可以说，不理解值类别，后面学引用、学移动语义、学完美转发的时候，你会一直处于"知道怎么写但不知道为什么"的状态。所以我们在这一章把它搞清楚，虽然不会讲得特别深（C++11 之后值类别的分类其实挺复杂的），但至少要把最核心的左值和右值搞明白，把引用的基本用法踩实。
...
你可以把左值想象成一个贴了标签的储物箱：箱子有自己的位置（内存地址，不管是栈上，还是堆上），标签让你**随时能找到它**（变量名），你往里面放东西、从里面拿东西都行。换而言之，当程序在跑的时候，您可以指出来一块内存，说：“呐，找我要x吗？去那看！”这种话。
...
```cpp
int x = 10;       // x 是左值，栈上一块小空间拿来放了10
int* ptr = &x;    // ptr 是左值，&x 取出了 x 的地址存放到了 ptr 这个内存小个格子了，你可以在内存上找到这个ptr。
*ptr = 20;        // 看看ptr指向啥？哦，他存储的是x的地址，我去看看x在哪，找到了，上面int x的时候编译器丢到栈的一个地方上，涂掉10改成20
int arr[5] = {};
...
别把"左值"和"能放在赋值号左边"画等号：`const int cx = 10;` 里 `cx` 是左值，但 `cx = 20;` 编译不过，const 左值不能被赋值。左值描述的是"有身份"（有内存地址），不是"能修改"。

## 什么是右值——短暂的临时存在

右值（rvalue）和左值正好相反：它是一个**没有持久身份**的表达式，通常是临时产生的值，你没法对它取地址，它在表达式求值完之后就可能消失。从这个事情上看，您可以理解为立即数一样的东西。用完就丢，你压根找不到这玩意最后在哪。
...
函数返回值的情况要特别注意：如果函数返回的是值（不是引用），比如 `int get_value() { return 42; }`，那调用 `get_value()` 得到的结果是右值，从语法层面上，**它是函数内部拷贝出来的一个临时值，没有持久身份**；但如果写的是 `int& get_ref() { return x; }`，`get_ref()` 返回的是引用，结果是左值——因为它最终绑定到了一个有身份的变量上。
...
> 个人觉得设计不太好，虽然方便，但是写代码的时候多了一个例外，没关系，我们后续学移动语义的时候，传递右值会匹配到更好的接口上。

## 引用基础——不是指针，胜似指针

既然提到了引用，我们就把它的基本用法讲清楚。引用在概念上很简单——它是一个已经存在的变量的**别名**。从你创建它的那一刻起，它就和被引用的变量绑定在一起，对引用做的任何操作都等同于对原变量操作。

> 好像指针哦！对！一种理解，也是我的理解，就是对指针的二次封装，让你不用写一大堆*了！
...
返回局部变量的引用是新手最容易犯的错误之一：局部变量在函数返回后就销毁了，引用指向的东西已经不存在，访问它就是**未定义行为**——可能得到垃圾数据，可能崩溃，也可能碰巧看起来正常，但无论哪种都是错的。
...
编译器通常会给出警告，但不会阻止你编译。记住一个简单的原则：**永远不要返回局部变量的引用或指针**。引用的规则到这里先建立一个能用的最小集；它和指针各管什么场景、怎么选，第 4 章会把两者放在一起系统对比。

...
C++ 给我们提供了一把安全锁：`const`。它的核心思路很简单——如果一个东西不应该变，那就明确告诉编译器，让编译器替我们看着。任何试图修改 `const` 值的代码，都会在编译阶段直接被拦下来。比起跑到线上才发现数据被意外篡改，在编译期就把问题掐死，显然靠谱得多。
...
constexpr int kResult = square(5);  // 编译期就算好了，kResult = 25
```

...
在栈上分配大数组是新手最常踩的坑之一。`int arr[10000000];` 这个看起来人畜无害的声明实际上需要约 40 MB 的栈空间——远超默认限制，程序启动时直接 segmentation fault，连个错误信息都来不及输出。如果你需要大块内存，请用 `std::vector` 或在堆上分配。
...
栈溢出（stack overflow）不像堆的 `new` 失败那样会抛出异常或返回 `nullptr`——操作系统检测到栈溢出时直接发送 SIGSEGV 信号，程序立刻终止。没有任何优雅处理的机会，调试时只能靠事后分析 core dump 或者在递归入口加计数器防护。
...
忘记 `delete` 导致内存泄漏（memory leak）是 C++ 最臭名昭著的问题之一。泄漏的内存在程序结束前永远不会被回收。对于短命的控制台程序这通常不是大问题（进程退出时操作系统会回收所有资源），但对于长时间运行的服务器程序或嵌入式系统，内存泄漏会一点一点把可用内存吃干，最终导致系统崩溃。这也是为什么我们在前面章节反复强调 RAII——用智能指针（`std::unique_ptr`、`std::shared_ptr`）和容器（`std::vector`、`std::string`）来管理动态内存，让析构函数自动释放资源，从根本上消灭手动 `delete` 的需求。
...
全局变量的构造和析构顺序在跨翻译单元（即不同 .cpp 文件）之间是未定义的。如果 `a.cpp` 里有个全局对象依赖 `b.cpp` 里另一个全局对象的初始化结果，程序可能会在启动阶段就出现未定义行为——因为标准不保证哪个先初始化。这就是臭名昭著的"静态初始化顺序陷阱"（Static Initialization Order Fiasco）。解决方案是使用函数内的 `static` 局部变量来包装（Construct On First Use Idiom），利用我们刚才说的延迟初始化特性来确保正确的初始化顺序。

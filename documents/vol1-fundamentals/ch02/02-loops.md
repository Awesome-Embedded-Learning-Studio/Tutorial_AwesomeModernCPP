...
什么时候用 `while`？最典型的场景就是"我们事先不知道要循环多少次，但是肯定知道退出的条件是什么的场景"。比如让用户不断输入数字累加，直到输入 0 为止：
...
循环体里面必须有能改变条件的操作（这里是我们每次重新读入 `value`），否则就会变成**死循环。**

**不是意图内的死循环是 `while` 最常见的坑**。如果循环体里没有任何操作能让条件变成 `false`，程序就会一直跑下去永远不退出。比如忘了写 `std::cin >> value;` 这一行，`value` 永远不变，条件永远为真。写 `while` 循环的时候，养成习惯检查"循环体里有没有改变条件的代码"。

## do-while 循环：先做一次再说的 while 循环变体
...
`do-while` 末尾的分号千万别忘。漏掉的话，编译器会把下一行代码当成 while 的循环体来解析，报错信息可能非常诡异。这是 C++ 里少数几个必须在 `}` 后面加分号的地方之一，和 `if`、`while`、`for` 都不一样，很容易搞混。

> PS，可能有朋友见过 `do { /* 一些代码 */  } while(0);` 这种代码宏，在C里非常常见，但是笔者要说明的是，在今天的编译器的环境下，这个写法纯粹多余。函数完全可以替代。
...
```cpp
for (int i = 1; i <= 10; ++i) {
    // 这个输出这样读：从 i = 1 开始，一直执行到i不再满足 i <= 10 这个关系，逼近的方式是i自增1.
    std::cout << i << " ";
}
// 注意，出了这个for循环，i 就没办法使用了！
...
差一错误（off-by-one error）是 `for` 循环最经典的坑。本意是循环 10 次，写成 `for (int i = 1; i < 10; ++i)` 就只跑了 9 次。一个实用建议：养成固定习惯。您有两个范式选。

- 要么总是从 0 开始用 `<`（`for (int i = 0; i < n; ++i)`）
- 要么从 1 开始用 `<=`（`for (int i = 1; i <= n; ++i)`）

**不要混用，混用就是差一错误的温床。**

## 循环中间的确需要打断了：那就用"紧急出口" break 和 continue
...
`break` 和 `continue` 用多了会让代码逻辑变得支离破碎，读代码的人得在脑子里跳来跳去地追踪执行流程。如果一个循环体里有超过两三个 `break` 或 `continue`，就该考虑是不是该把循环条件写得更清晰，或者把部分逻辑抽成单独的函数。简单直接的循环条件永远比到处 `break` 更容易维护。

...
C++17 之前，非 `const` 整型的静态成员变量必须在类外定义。如果你在头文件里声明了 `static int count_;` 却忘了在对应的 `.cpp` 文件里写 `int MyClass::count_ = 0;`，每个包含这个头文件的翻译单元都能编译通过，但最终链接时会炸。而且错误信息的措辞往往很抽象，新手根本不知道在说什么。
...
如果你的静态成员涉及拷贝或移动语义，一定要小心。默认的拷贝构造函数会逐成员拷贝，但它不会拷贝静态成员——因为静态成员不属于对象。如果你期望通过"拷贝一个对象来复制整个类的状态"，那这个设计就有问题了。静态成员的值不受任何单个对象的创建、拷贝或销毁影响（除非你在构造/析构函数里显式修改了它）。

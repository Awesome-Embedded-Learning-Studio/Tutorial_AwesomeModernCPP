...
这里有一个新手很容易踩的坑：**两个整数做除法，结果还是整数**。$\frac{10}{3}$ 不是 `3.333...`，而是 `3`。小数部分被直接丢弃了，不是四舍五入。

> ⚠️ **踩坑预警**
> 如果你想要得到带小数的除法结果，至少有一个操作数必须是浮点数。$\frac{10}{3}$ 得 `3`，但 $\frac{10.0}{3}$ 或 $\frac{10}{3.0}$ 得 `3.333...`。

### 负数除法：向零取整

C99 标准明确规定：整数除法向零取整。也就是说，结果的小数部分被丢弃后，结果趋近于零。$\frac{7}{2}$ 是 `3`，$\frac{-7}{2}$ 是 $-3$（不是 $-4$）。取模运算的余数符号和被除数相同：`-7 % 2` 是 $-1$。
...
关键点：C99 起 `/` 和 `%` 都向零取整（truncation toward zero），正负数规则一致，直接丢弃小数部分。所以 $\frac{-7}{2}$ 得 $-3$ 而不是 $-4$，`-7 % 2` 的符号随被除数取负。

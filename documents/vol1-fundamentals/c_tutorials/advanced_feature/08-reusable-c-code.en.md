...
Anyone who has written tens of thousands of lines of C code has probably experienced this—at the start of a project, everything is fine; a few ``.c`` files cobbled together and it runs. But as features pile up, the code turns into a tangled mess: header files include each other indiscriminately, global variables are everywhere, changing a single struct field triggers recompilation of a dozen source files, and just when you finally get it working on a PC, porting it to an STM32 brings a whole new set of issues. Honestly, the root cause of this pain usually isn't a flawed algorithm or a blown-up pointer—it's failing to take "code organization" seriously from day one.
...
But this doesn't mean you can't write clean, modular code in C—it just means we need to manually achieve what other languages do automatically. Understanding these manual techniques is crucial, because C++'s ``namespace``, ``class`` access control, PIMPL idiom, and even C++20 Modules are all engineering upgrades built on top of these C practices. Once you understand the C approach, you'll truly grasp why C++ is designed the way it is.
...
> ⚠️ **Pitfall Warning**
> Exposing the full struct definition in a header means callers will eventually be tempted to directly access internal fields. Once you modify the struct layout, every source file that includes this header must be recompiled—in a large project, this could mean minutes of compilation time. Even worse, callers might already be relying on your internal implementation, making it impossible to change.
...
In the header file, we didn't expose the internal structure of ``RingBuffer``—``typedef struct RingBuffer RingBuffer;`` is just a forward declaration plus a typedef. The caller only gets a ``RingBuffer*`` pointer, and then manipulates it through the functions we provide. They don't know whether the buffer is implemented with an array or a linked list—they know nothing, and that's exactly right.
...
This pattern is extremely common in embedded systems. Through conditional compilation, the same codebase can adapt to resource-constrained MCUs (disabling unnecessary features to save Flash and RAM) and feature-rich Linux environments.
...
Runtime configuration is passed through function parameters or configuration structs, and is suitable for parameters that are only determined at program startup or might change during execution. The ``UartConfig`` struct in the UART driver earlier is a typical example of runtime configuration.
...
The core technique for making code run on multiple platforms is introducing a Platform Abstraction Layer. The principle is simple: **isolate all platform-specific code in one place, and have upper-layer code only call abstract interfaces**. You can think of it like a universal charger—whether your phone uses USB-C or Lightning, plug in the adapter and it charges; the adapter is the "platform abstraction layer."

Suppose our ring buffer needs to use a fixed-size static array on embedded platforms (no ``malloc``), but can use dynamic allocation on a PC. We first define a set of platform interfaces:
...
Cross-platform code also has a common type trap: the size of fundamental types may differ across platforms. ``int`` might be 16-bit on an 8-bit MCU but 32-bit on a 32-bit platform, and ``long`` is 64-bit on 64-bit Linux but 32-bit on Windows. So cross-platform code should uniformly use the fixed-width types defined in ``<stdint.h>``: ``uint8_t``, ``uint16_t``, ``uint32_t``, ``size_t``, and so on.

## Step Six — Evolve Your API Stably

When your module is used by multiple projects, API stability becomes an issue you must take seriously. Changing a function name or adding a parameter means all callers have to follow suit—if you don't control those callers, that's a disaster.
...
If an old interface truly needs to be deprecated, you can first mark it with a macro, giving users a transition buffer period:
...
The modular techniques we've been working through in C all have more powerful native support in C++. Understanding the C approach helps us grasp the design motivation and underlying mechanisms of C++ tools—every "new feature" in C++ wasn't invented out of thin air; they are engineering upgrades built on top of C's manual practices.
...
| Naming prefixes (``ringbuf_``, ``uart_``) | ``namespace`` | True namespace isolation, no manual prefix writing |
...
C++20 introduced the Modules system, designed to fundamentally replace the header file's ``#include`` mechanism. Modules directly solve many inherent problems of header files—macro pollution, redundant parsing, and fragile dependency order. That said, as of late 2024, mainstream compiler support for modules is still rapidly evolving, and adopting modules in large projects requires significant migration effort. But it's worth understanding as a trend; we won't expand on it here (the upcoming C++ advanced volume will cover it in detail).
...
Please implement two versions: one using the standard library's ``malloc``/``free`` (suitable for PC), and another using a static memory pool (suitable for bare-metal embedded environments). The hash table's ``.c`` file should allocate memory by including ``pal.h``, rather than calling ``malloc`` directly.

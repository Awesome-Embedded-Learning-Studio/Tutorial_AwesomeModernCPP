...
声明函数指针时，`(*op_ptr)` 外面的括号**绝对不能省**。省掉就变成了返回指针的函数声明，编译器不会报错，但行为完全不同。这是新手最容易犯的错误之一。
...
使用函数指针数组做分发时，一定要检查索引是否越界。如果 `cmd_id` 超出数组范围，访问到的要么是垃圾地址，要么是 NULL——直接调用就是段错误（segmentation fault）。
...
`qsort` 的比较函数接收的是 `const void*`，返回值遵循"左小于右返回负数，相等返回 0，左大于右返回正数"的约定。如果你把比较逻辑写反了，排序结果就是乱序——而且不会有任何编译期提示。
...
示例里的 `device_ready` 用 `static` 局部变量模拟外设状态，它的值

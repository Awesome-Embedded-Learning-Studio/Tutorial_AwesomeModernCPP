...
关系比较也是同样的思路。数学上 `a/b < c/d` 等价于 $a\times d < c\times b$（假设分母都是正数，`normalize()` 已经保证了这一点），然后 `>`、`<=`、`>=` 全部基于 `<` 来派生：
...
所有运算结果都正确。`a + b` 得到 $\frac{5}{6}$（通分后 `3/6 + 2/6`），除法 `1/2 / 1/3` 得到 $\frac{3}{2}$，混合运算 $\frac{2 \times 1}{3}$ 也正常工作——`2` 被隐式转换为 `Fraction(2, 1)` 然后参与乘法。约分在每个运算步骤中都自动完成了，这是 `normalize()` 的功劳。

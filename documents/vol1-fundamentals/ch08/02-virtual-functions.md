...
不加 `override` 不会报错，但签名一错就是灾难。养成习惯：每个虚函数重写都加 `override`，把它当作和系安全带一样的强制动作。
...
在 RAM 只有几 KB 的 MCU 上，每个对象多一个 `vptr` 可能是致命的。如果你的系统需要创建大量小对象（比如传感器采样数据点），请认真评估多态的内存开销。
...
非虚析构函数 + 通过基类指针 delete 派生类对象 = 未定义行为。在嵌入式中这通常表现为"莫名其妙的内存泄漏"或"外设状态异常"，而且定位起来异常困难。看到虚函数，立刻检查析构函数是不是也是虚的。

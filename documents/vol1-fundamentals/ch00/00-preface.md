...
先交代一下背景。这套教程的起点其实是一个很私人的动机——我在做嵌入式开发的过程中，越来越觉得纯 C 写到后面力不从心。手动管理资源、到处传递回调函数指针、用宏做泛型——这些套路用久了，代码膨胀得让人头疼，维护成本也越来越高。我就在想，有没有一种方式，既不丢掉 C 那种"贴近硬件"的控制力，又能用上更现代的语言特性来组织代码？答案当然就是 C++，而且不是那种九十年代的"C with Classes"，而是从 C++11 开始一路进化到 C++23 的现代 C++。

但当我真正开始学的时候，发现一个很尴尬的事情：市面上大多数 C++ 教程要么太老，还在花大篇幅讲 C++98 时代的陈旧实践；要么太偏理论，读完之后仍然不知道怎么下手写一个真实的程序。嵌入式方向的 C++ 资料更是凤毛麟角，偶尔找到几篇也大多是浅尝辄止。所以我决定自己来——把自己折腾 C++ 的完整学习路径记录下来，从基础语法一直写到实战应用，尽量让后来者少走弯路。

这就是这套教程诞生的原因。它不是一本语言律师手册，也不是一份官方标准文档的翻译——它是一位工程师的学习笔记，记录了从零开始掌握 C++ 的完整旅程。
...
浏览器可能是 C++ 最成功的应用场景之一。Chrome 的渲染引擎 Blink、Firefox 的 Gecko、Safari 的 WebKit——这些每天被数十亿人使用的软件，全部是 C++ 写的。浏览器需要做的事情极其复杂：解析 HTML 和 CSS、执行 JavaScript、渲染页面、管理网络请求和缓存，同时还得保持 60fps 的流畅度，这对语言的性能要求近乎苛刻。
...
而在嵌入式领域——这是这套教程特别关注的场景——从 STM32 单片机上的外设驱动，到 RTOS 的任务调度，到 Linux 嵌入式上的系统级编程，C++ 正在逐步替代传统的纯 C 方案，因为现代 C++ 提供的类型安全和零开销抽象，在资源受限的环境中格外有价值。

这些不是什么冷知识，但合在一起看，你会发现一个事实：从你打开手机的那一刻起，到你浏览网页、玩一局游戏、刷一张银行卡，你的生活中到处都有 C++ 的影子在默默运行。
...
**把代码自己敲一遍。** 这不是客套话。看懂一段代码和亲手把它敲出来、编译运行、看到输出结果，完全是两种不同的学习体验。你会在敲代码的过程中发现各种意想不到的小问题——拼写错误、忘记加分号、头文件没包含——这些都是真实编程的一部分，早点遇到、早点习惯，比什么都强。所以哪怕教程里的示例代码看起来再简单，也请你自己动手敲一遍。

**做练习，不要偷懒。** 每篇文章后面如果附有练习题，请务必认真完成。这些练习不是为了凑字数，而是帮你把"看懂了"转化为"会用了"。很多时候你觉得自己理解了一个概念，但一到写代码就卡住——这就是练习的价值所在。

**遇到不懂的地方先自己思考，但不要死磕。** 如果某个概念看了两三遍还是不理解，先标记下来继续往后读。很多概念在后续的实际应用中会变得清晰起来，因为上下文变了，你的理解也会随之加深。但如果回过头来还是不懂，可以去找社区讨论或者查阅 cppreference.com 上的详细说明。

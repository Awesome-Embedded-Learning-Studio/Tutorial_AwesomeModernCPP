...
变量声明那里，我们用了 `int a = 0;` 而不是 `int a;`，这是故意的。C++ 不会自动给局部变量初始化——如果不赋初值，`a` 的值就是内存里残留的垃圾数据（静态局部变量和全局变量是例外，它们会被自动零初始化）。虽然后面 `std::cin` 会立刻覆盖它，但养成"声明即初始化"的习惯非常重要，这能帮你避开一大类难以调试的问题。

...
环境搭好了，编译器也装好了，接下来该干正事了——写我们的第一行 C++ 代码。说实话，写一个 "Hello World" 听起来是最无聊的事情，但我可以保证，如果我们把这个小程序拆清楚，后面很多概念都会变得顺理成章。所以别急着跳过，我们一行一行地把它吃透。

## 从零开始——hello.cpp 的骨架

打开你喜欢的编辑器，新建一个文件叫 `hello.cpp`，把下面这段代码原封不动地敲进去。注意是敲进去，不是复制粘贴——肌肉记忆这个东西，在学编程的时候真的很重要。
...
修改 `hello.cpp`，让程序输出你的名字而不是 "Hello, C++!"。比如输出 "你好，我是小明！"。

...
变量声明那里，我们用了 `int a = 0;` 而不是 `int a;`，这是故意的。C++ 不会自动给局部普通变量初始化(静态局部变量，全局变量自动初始化为`0`，)——如果不赋初值，`a` 的值就是内存里残留的垃圾数据。虽然后面 `std::cin` 会立刻覆盖它，但养成"声明即初始化"的习惯非常重要，这能帮你避开一大类难以调试的问题。

>拓展:注意动态分配的内存是否自动初始化为`0`与创建方式有关，`int* p = new int;`不会初始化，`int* p = new int{};`,`int* p = new int();`会初始化为0,动态分配的数组同理.

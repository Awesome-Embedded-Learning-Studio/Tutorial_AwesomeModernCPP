...
写过模板函数的朋友大概都撞上过这个困境：接收一个参数，再把它传给另一个函数，传左值进去希望对方收到左值，传右值进去希望对方收到右值。听起来简单，对吧？但在 C++11 之前这几乎做不到，要么写两个重载（一个收左值引用，一个收右值引用），要么干脆全用 const 引用接收，把"这是右值"的信息丢了，移动语义的性能优势也没了。天啊，效率和性能这下没法兼顾，烦人！

不过没事，C++11 同期给出的完美转发（perfect forwarding）就是来解决这个问题的。它让咱们只写一个模板，就能把参数的值类别原封不动地"转发"给目标函数。

一句话：以前参数转交到别处总得写 const T& 和 T&& 两份，现在不用了，用 std::forward 转发（或者说透传）。

## 从一个实际的问题开始

假设咱们在写一个简单的工厂函数，用来创建 `std::string` 对象：
...
版本一能接受左值，但传入右值时也会拷贝，因为您用 const 引用接收，丢掉了"这是一个右值"的信息。版本二能接受右值并正确移动，但传入左值时编译直接报错，因为右值引用不能绑定左值。
...
两个参数呢？四个重载（const& + const&、const& &&、&& const&、&& &&，2×2）。三个参数就八个。完蛋，工程里一堆成员要处理，这么写非炸不可，显然没法持续下去。

## 万能引用——不是所有 T&& 都是右值引用

Scott Meyers 给这种特殊的 `T&&` 起了名字叫"万能引用"（universal reference），C++ 标准术语叫"转发引用"（forwarding reference）。它长得和右值引用一模一样（欸，笔者也有点想不通为什么，不知道有没有 C++ 大佬能给讲讲为啥偏偏长一样，小生受教！），但行为完全不同。
...
万能引用的出现有两个必要条件，缺一不可：第一，类型必须通过模板参数推导（`template<typename T>` 中的 `T`）；第二，声明形式必须恰好是 `T&&`，不能加 const 或其他修饰。您若写 `const T&&`，那它就是普通的 const 右值引用，不是万能引用。您若写 `std::vector<T>&&`，也不是万能引用，`T` 虽然被推导了，但 `std::vector<T>&&` 这个整体不是 `T&&` 的形式。
...
这一部分大量参考了《Effective Modern C++》：

万能引用之所以能工作，靠的是**引用折叠**（reference collapsing）。当编译器推导 `T&&` 的时候，可能出现"引用的引用"，比如 `T` 被推导为 `std::string&`，那 `T&&` 就变成了 `std::string& &&`。C++ 不允许直接写"引用的引用"，但在模板推导的上下文中，编译器会按四条规则来折叠它：

`T& &` 折叠为 `T&`，`T& &&` 折叠为 `T&`，`T&& &` 折叠为 `T&`，`T&& &&` 折叠为 `T&&`。

不用死记这四条，记一个简洁的规律就够：**只要有一个是左值引用（`&`），结果就是左值引用**。只有两个都是右值引用（`&& &&`），结果才是右值引用。

咱们用具体的推导过程验证一下。传入左值 `name` 时，`T` 被推导为 `std::string&`，于是 `T&&` 变成 `std::string& &&`，按第二条规则折叠为 `std::string&`，参数类型是左值引用。传入右值 `std::string("Bob")` 时，`T` 被推导为 `std::string`（非引用类型），于是 `T&&` 就是 `std::string&&`，参数类型是右值引用，没有折叠发生，因为原本就没有"引用的引用"。
...
引用折叠不只出现在函数模板中。`auto&&` 的推导、`typedef` 和 `using` 别名的实例化、以及 `decltype` 的某些用法中都会触发引用折叠。不过函数模板中的万能引用是最常见的场景。

## std::forward——有条件的类型转换

好，这里才是重点（如果您只关心怎么用的话）。理解了万能引用和引用折叠，`std::forward` 就很简单了。它的作用是：**传入的是右值时，把参数转成右值引用；传入的是左值时，保持左值引用不变**。本质上是一个有条件的、更聪明的 `static_cast`。（一句话，嘿，这小东西记得您传的是左值还是右值，原样透传到别处去了。）

值类别在哪一步丢掉、`std::forward` 又怎么把它原样保住，做成了动画，您可以播放、暂停，也可以按步进键单步看，把左值和右值两条路径各走一遍：

<Anim id="perfect-forwarding" />

咱们可以自己实现一个简化版本来理解它的原理：
...
这两个重载配合引用折叠完成了"有条件转换"的逻辑。传入左值时，`T` 被推导为 `U&`（U 是实际类型），`static_cast<T&&>` 就是 `static_cast<U& &&>`，折叠为 `U&`，返回左值引用。传入右值时，`T` 被推导为 `U`，`static_cast<T&&>` 就是 `static_cast<U&&>`，返回右值引用。

关键的洞察在于：`std::forward` 的"条件性"来自**模板参数 `T` 携带了原始实参的值类别信息**，不在 `std::forward` 自身的逻辑里。当万能引用接收左值时，`T` 被推导为 `U&`，这个 `&` 就像一枚印章，把"这是左值"的信息刻在了类型里。`std::forward` 通过 `static_cast<T&&>` 和引用折叠把这枚印章"解印"出来。
...
这里的 `Args&&... args` 是万能引用的参数包。每个 `Args` 独立推导，所以如果您传入一个左值和一个右值，它们各自的值类别都被保留。`std::forward<Args>(args)...` 把每个参数按照它原始的值类别转发给 `T` 的构造函数。
...
另一个经典例子是 `std::vector::emplace_back`。它接收的是构造参数，不是现成对象，直接在 vector 的内存里原位构造新元素，比 `push_back` 更高效，连移动都省了。
...
如果 `x` 是右值引用，第一次 `std::forward<T>(x)` 会把 `x` 转成右值传递给 `target`，`target` 可能已经偷走了 `x` 的资源。第二次再 forward 时，`x` 已经处于"有效但未指定"的状态，您把一个可能已经空了的右值传了出去。这就是所谓的"use-after-move"，虽然编译器不会报错，但运行时行为不可预测。
...
这里的 `decltype(auto)` 会根据 `return` 表达式推导返回类型，所以返回类型取决于 `std::forward<T>(x)` 的结果。当您传入一个右值时，`T` 被推导为非引用类型（比如 `std::string`），`std::forward<std::string>(x)` 返回 `std::string&&`，`decltype(auto)` 推导出的返回类型就是 `std::string&&`。但这个右值引用指向的是函数参数 `x`，`x` 在函数返回时就销毁了。调用者拿到的引用指向了一块已经不存在的内存，经典的悬空引用，GCC 的 `-Wdangling-reference` 会对此发出警告。
...
如果您真的需要在返回语句中做转发，确保返回类型是值类型（`T` 而不是 `decltype(auto)`），这样右值场景下会触发移动构造而不是返回引用。上一节缓存包装器中的 `emplace_get` 就是一个正确的例子：它返回 `Value&`（固定类型，不是转发来的），只对参数使用 `std::forward`。

## 通用示例——通用的缓存包装器

咱们用完美转发来写一个实用的例子：一个通用的缓存包装器模板，它可以缓存任意函数调用的结果，并且完美转发所有参数。
...
`emplace_get` 中的 `Args&&... args` 是万能引用参数包。当您传入 `("first", 100)` 时，`Args` 被推导为 `const char (&)[6]` 和 `int`（近似理解为 `const char*` 和 `int`）。`std::forward<Args>(args)...` 把这些参数原封不动地转发给 `ExpensiveData` 的构造函数，构造函数拿到的参数类型和值类别与您直接传给它时完全一致。
...
为了加深理解，咱们来写一个小程序，用 `std::is_same_v` 来验证引用折叠的结果：

...
关于 `operator*` 和 `value()` 的选择，笔者的建议是：在你**已经检查过** `has_value()` 的代码路径中，使用 `*opt` 就够了，性能更好而且语义清晰。在**没有检查**的情况下，`value()` 更安全——它会抛异常而不是 UB。但这两种方式都不如 `value_or()` 来得优雅，因为后者直接处理了"空值怎么办"的问题。

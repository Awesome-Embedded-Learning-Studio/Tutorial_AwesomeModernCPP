...
笔者写这篇文章之前，翻了一下以前写的 C 风格代码——满屏幕的 `enum Color { Red, Green, Blue };`，然后 `if (color == 1)` 这种东西随处可见。说实话，到了 2026 年还这么写，基本上就是在给自己挖坑。C 风格 enum 的隐式整数转换、命名污染、无法前向声明，这三板斧砍下来，每一条都够在 code review 里被骂一顿。

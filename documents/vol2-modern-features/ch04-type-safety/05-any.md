...
笔者第一次接触 `std::any` 的时候，反应是：这不就是 `void*` 的包装吗？有什么用？后来在做一个插件系统的配置模块时才意识到——`void*` 丢掉了所有类型信息，你从它里面取值的时候完全靠猜；而 `std::any` 虽然也能装任何类型，但它**记得**自己装的是什么。当你用错误的类型去取值时，它会抛异常而不是给你一段内存垃圾。

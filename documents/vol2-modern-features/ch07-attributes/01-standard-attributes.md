...
本文重点展开的四个属性分成两类：前两个不写时编译器一声不吭、写上后立刻警告，后两个不写时误报打扰、写上后恢复安静：

![四大标准属性效果对照](./01-standard-attributes-effects.drawio)
...
注意：如果你给一个实际上会返回的函数加了 `[[noreturn]]`，行为是未定义的。编译器可能不会报错，但生成的代码可能完全不符合预期。

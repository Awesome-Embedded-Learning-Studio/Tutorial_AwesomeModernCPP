...
在逐个拆解之前，先把三个性能属性放进同一张效果卡对照：

![C++20-23 性能属性效果卡：干什么、省什么、坑在哪](./02-modern-attributes-effects.drawio)
...
注意属性的放置位置：`[[likely]]` 放在分支体的 `{` 前面，不是放在条件表达式上。这是 C++20 标准的规定。
...
警告：`[[assume]]` 是所有属性中最危险的。如果你的假设是错的，程序的行为完全不可预测。笔者建议只在经过充分 profiling、确认瓶颈、并且你能 100% 保证条件总是成立的情况下使用它。在 99% 的代码中，你不需要它。

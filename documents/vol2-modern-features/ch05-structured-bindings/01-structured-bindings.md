...
`result.second` 是什么意思？不查文档您根本不知道。结构化绑定直接把语义写进变量名里：
...
注意结构化绑定只支持一维数组的直接解包。您不能写 `auto [a, b, c, d, e, f] = matrix`，因为 `matrix` 本质上是 `int[2][3]`，大小是 2 而不是 6。
...
如果您的类有私有成员，不能直接用结构体方式解绑。但 C++ 提供了另一条路：让编译器把您的类当作 "tuple-like" 类型来处理。只需要三样东西：
...
`auto&&` 绑定到临时对象时，匿名变量的生命周期会被延长到绑定变量的作用域结束，所以用 `auto&&` 或 `const auto&` 是安全的。但如果您拿到了绑定变量的指针或引用然后传出去，就有悬空风险：
...
结构化绑定的变量名不能直接用于函数返回。如果您想返回解包后的值，需要重新打包：
...
如果您需要存储解包后的值，用结构体或者 `pair`/`tuple` 成员代替。

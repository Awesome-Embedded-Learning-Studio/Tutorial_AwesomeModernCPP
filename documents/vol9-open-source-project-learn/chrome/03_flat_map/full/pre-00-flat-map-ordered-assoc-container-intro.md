...
Chromium 在 `//base` 里另起了一套有序关联容器——`flat_map`、`flat_set`——路子完全反过来:用一段连续的有序数组装所有元素,查找走二分。渐近复杂度还是 $O(\log n)$,但数据挨在一起,CPU 一条 cache line 顺带就把十几个元素全捎进 L1 了,常数因子小一大截。代价当然有,$O(n)$(要挪数组)。咱们这一篇就把 std::map 的红黑树拆开看,讲清楚 flat_map 到底为什么要换这条路。
